...
export const sendOTP = async (req, res) => {
    ...
    try {
        ...
        if (!verificationToken || Date.now() > user.verificationTokenExpiresAt) {
            return res.status(400).json({
                success: false,
                message: "Verification token is missing or expired. Please register again.",
            });
        }

        // Send the token via Twilio SMS
